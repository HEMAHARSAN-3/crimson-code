      ...
      <div className="mt-10 grid gap-px border border-border bg-border md:grid-cols-2 lg:grid-cols-4">
        {snapshot.map((s, i) => (
          <Reveal key={s.number} delay={i * 0.05}>
            <div className="h-full bg-background p-5 transition-colors duration-300 hover:bg-card sm:p-6 lg:p-8">
              <span className="display text-3xl font-bold text-transparent [-webkit-text-stroke:1px_var(--primary)]">
                {s.number}
              </span>
              <h3 className="display mt-6 text-lg font-semibold uppercase">{s.title}</h3>
        ...
        <ol className="mt-10 grid gap-px bg-border sm:grid-cols-2 lg:grid-cols-3">
          {processSteps.map((s, i) => (
            <Reveal key={s.label} delay={i * 0.05}>
              <li className="group relative h-full bg-background p-5 sm:p-6 lg:p-8">
                <span className="eyebrow text-primary">{String(i + 1).padStart(2, "0")}</span>
                <p className="display mt-5 text-2xl font-semibold uppercase">{s.label}</p>
